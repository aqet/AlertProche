...
export class PostFormComponent implements OnInit {
  ...
  camerounCities = [
    ...
    'Limbe', 'Kumba', 'Edéa', 'Loum', 'Nkongsamba', 'National'
  ];
  ...
  processFile(file: File) {
    ...
    if (file.size > this.MAX_SIZE) {
      this.imageError.set('L\'image dépasse 5 Mo. Veuillez choisir une image plus légère.');
      return;
    }
  ...
  getFileSizeLabel(): string {
    ...
    return size > 1024 * 1024 ? `${(size / 1024 / 1024).toFixed(1)} Mo` : `${Math.round(size / 1024)} Ko`;
  }

  onSubmit() {
    if (this.form.invalid) { this.form.markAllAsTouched(); return; }
    this.loading.set(true); this.error.set('');
    this.postService.createPost(this.form.value, this.selectedFile() || undefined).subscribe({
      next: (post) => { this.loading.set(false); this.success.set(true); this.router.navigate(['/posts', post._id]); },
      error: (err) => {
        this.loading.set(false);
        this.error.set(err?.error?.message || 'Une erreur est survenue. Vérifiez votre texte et réessayez.');
      }
    });
  }
